...
def build_fin_model(user_id: int, db_path: Path = DB_PATH, cache_path: Path = CACHE_PATH) -> dict:
    ...
    def to_rub(amt, code):
        if code == "RUB":
            return amt
        r = code_to_usd.get(code)
        return amt * r / rub_usd if r else 0.0

    exp_by_month: dict[str, float] = {}
    for r in cur.fetchall():
        exp_by_month[r["month"]] = exp_by_month.get(r["month"], 0) + to_rub(r["amt"], r["code"])
    avg_expenses = round(sum(exp_by_month.values()) / len(exp_by_month)) if exp_by_month else 0
